...
foreach($tokens as $tokenKey => $tokenValue) {
	if($tokenKey == "aa_ack_time") {
                $sql ="update aem_alert set aa_ack_time=NOW() where aa_id=".$aem_incident;
                mysql_query($sql,$aem) or handleError("update - aa_ack_time",mysql_error()." - ".$sql);
                continue;
        }
