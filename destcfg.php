...
elseif (!isset($_REQUEST['continue']) or $_REQUEST['continue'] != "true")
 {
...
while ($row = mysql_fetch_assoc($res))
 {
  ...
  echo("</td>");
  echo("<td>");
  $sql="select max(".$db_col_seq.") max from ".$db_tbl_stage." where ".$db_col_src."='".$source."' and ".$db_col_type."='".$type."'";
  $maxres=mysql_query($sql,$aem) or die(mysql_error());
  $max=mysql_fetch_assoc($maxres);
