			...
			if($_REQUEST['pg'] == 'steps'){
				...
				if($_POST['renaming']){
					if($_POST['step_type'] == 'script'){
						...
						$renameAction=$_POST['step_type'];
					}
					$sql="UPDATE aem_step SET as_name = '".$_POST['step_name']."', as_action='".$renameAction."', as_return_token=".$_POST['return_token']." WHERE as_id =".$_POST['renameID']."";
					print $sql.'<br>';
					$qry=mysql_query($sql, $aem);
				}
				if($_GET['copying']){
				...
				if(!$_GET['chg_token']){
				  ...
				  if($_GET['rename']){
					 ...
					 $message=mysql_result($qryren,0,'as_action');
					 $art=mysql_result($qryren,0,'as_return_token');
					 if($message == 'function blackout'){
						$selectB='selected="selected"'; 
					 }
					 if($message == 'function translate'){
						$selectT='selected="selected"'; 
					 }
					 if($message != 'function blackout' && $message != 'function translate'){
